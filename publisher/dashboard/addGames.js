...
export default function AddAdmin() {
  ...
  const onSubmit = async (data) => {
    ...
    formData.append("yearOfRelease", data.password);
    formData.append("myfile", data.myfile[0]);

    try {
      const response = await axios.post(
        "https://nestjsproject-production-364f.up.railway.app/admin/insertadmin",
        formData,
        {
          headers: {
      ...
      setSuccess("games added successfully");
      reset();
    } catch (error) {
      setSuccess("Games added failed " + error.response.data.message);
    }
  };
